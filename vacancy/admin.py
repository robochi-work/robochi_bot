...
from .choices import STATUS_APPROVED, STATUS_PENDING
...
from .services.observers.call_observer import (
...
from .services.observers.events import (
...
from .tasks.call import after_first_call_check, before_start_call, close_vacancy, final_call_check, start_call_check
...
class StatusDefaultFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        total = qs.count()
        choices = [("all", _("{label} ({count})").format(label=_("All"), count=total))]
        # По каждому статусу
        for value, label in Vacancy._meta.get_field("status").choices:
            count = qs.filter(status=value).count()
            choices.append((value, _("{label} ({count})").format(label=label, count=count)))
        return choices
...
class VacancyAdmin(admin.ModelAdmin):
    ...
    actions = [
        ...
        "before_start_call_action",
        "start_call_action",
        "final_call_action",
        "after_first_call_check_action",
        "send_start_call",
        "send_final_call",
        "close_vacancy_action",
    ...
    def get_fieldsets(self, request, obj=None):
        fieldsets = [
            (_("Vacancy Info"), {"fields": ("owner", "gender", "people_count", "has_passport")}),
            (_("Location and Time"), {"fields": ["address", "date_choice", "date", "start_time", "end_time"]}),
            (_("Payment and Skills"), {"fields": ("payment_amount", "payment_unit", "payment_method", "skills")}),
            (
                _("Status"),
                {
                    "fields": (
        ...
        if obj:
            for _name, section in fieldsets:
    ...
    def save_model(self, request, obj: Vacancy, form, change):
        ...
        update_vacancy_status(
            obj, new_status=obj.status, old_status=form.initial.get("status"), changed_by=request.user
        )
    ...
    def notify_owner(self, request, queryset):
        for vacancy in queryset:
            VacancyCreatedUserObserver(telegram_notifier).update(
                VACANCY_CREATED,
                data={
                    "vacancy": vacancy,
                },
            )
        self.message_user(request, gettext("Notification sent to owner(s)."), messages.SUCCESS)

    @admin.action(description=_("Update message in channel - vacancy is full"))
    def update_channel_message(self, request, queryset):
        for vacancy in queryset:
            VacancyIsFullObserver(telegram_notifier).update(
                "test_event",
                data={
                    "vacancy": vacancy,
                },
            )
        self.message_user(request, gettext("Updated."), messages.SUCCESS)

    @admin.action(description=_("Send notification about creation to admins"))
    def notify_admins(self, request, queryset):
        for vacancy in queryset:
            VacancyCreatedAdminObserver(telegram_notifier).update(
                VACANCY_CREATED,
                data={
                    "vacancy": vacancy,
                },
            )
        self.message_user(request, gettext("Notification sent to admin(s)."), messages.SUCCESS)
    ...
    def before_start_call_action(self, request, queryset: Iterable[Vacancy]):
        ...
        self.message_user(request, gettext("call."), messages.SUCCESS)

    @admin.action(description=_("start call to owner"))
    def start_call_action(self, request, queryset: Iterable[Vacancy]):
        for vacancy in queryset:
            VacancyStartCallObserver(notifier=telegram_notifier).update(
                VACANCY_START_CALL,
                data={
                    "vacancy": vacancy,
                },
    ...
    def final_call_action(self, request, queryset: Iterable[Vacancy]):
        for vacancy in queryset:
            VacancyAfterStartCallObserver(notifier=telegram_notifier).update(
                VACANCY_AFTER_START_CALL,
                data={
                    "vacancy": vacancy,
                },
        ...
        for vacancy in queryset:
            if vacancy.group:
                if vacancy.group.invite_link:
                    ...
                    self.message_user(request, gettext("The vacancy group does not have a invite link"), messages.ERROR)
            else:
                self.message_user(request, gettext("The vacancy does not have a group"), messages.ERROR)
        self.message_user(request, gettext("Vacancies sent to channel(s)."), messages.SUCCESS)
    ...
    def check_before_20_start(self, request, queryset):
        ...
        self.message_user(request, gettext("Vacancies sent check to members."), messages.SUCCESS)
