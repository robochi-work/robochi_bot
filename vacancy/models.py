...
from telegram.models import Channel, ChannelMessage, Group
...
from .services.observers.events import VACANCY_DELETE

User = get_user_model()


class Vacancy(models.Model):
    owner = models.ForeignKey(User, on_delete=models.CASCADE, related_name="vacancies", verbose_name=_("Owner"))
    ...
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default=STATUS_PENDING, verbose_name=_("Status"))
    ...
    @property
    def last_channel_message(self) -> ChannelMessage | None:
...
class VacancyUserCall(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=CallStatus.choices, default=CallStatus.SENT, verbose_name=_("Call status")
    )
    call_type = models.CharField(max_length=20, choices=CallType.choices, verbose_name=_("Call type"))
    updated_at = models.DateTimeField(auto_now=True, verbose_name=_("Updated at"))
    created_at = models.DateTimeField(default=timezone.now, blank=True, null=True, verbose_name=_("Created at"))
    ...
    def __str__(self):
        return f"{self.vacancy_user} [{self.call_type}]"


class VacancyStatusHistory(models.Model):
    vacancy = models.ForeignKey(
        "Vacancy", on_delete=models.CASCADE, related_name="status_history", verbose_name=_("Vacancy")
    )
